...
export default function NotesForm({ onClose }: { onClose?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation logic matching Zod schema
    ...
    if (formData.program.length < 2 || formData.program.length > 100) {
      toast.error("Please select a valid program");
      setLoading(false);
      return;
    }
    if (
      formData.courseCode.trim().length < 2 ||
      formData.courseCode.length > 20
    ) {
      toast.error("Course Code must be between 2 and 20 characters");
      setLoading(false);
      return;
    }
    if (
    ...
    try {
      ...
      console.log("Submitting Notes Data:", formData);
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-gray-800">Upload Notes</h2>
        {onClose && (
          <button
            onClick={onClose}
